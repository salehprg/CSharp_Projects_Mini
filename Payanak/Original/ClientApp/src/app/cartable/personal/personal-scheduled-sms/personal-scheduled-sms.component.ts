...
@Component({
  selector: 'app-personal-scheduled-sms',
  templateUrl: './personal-scheduled-sms.component.html',
  styleUrls: ['./personal-scheduled-sms.component.scss']
})
export class PersonalScheduledSmsComponent implements OnInit, OnDestroy {
  ...
  edit(ssi: ScheduleSmsInfoModel) {
    const ref = this.modalService.open(AddScheduleSmsModelComponent, {size: 'lg'});
  ...
  lunchAddSS() {
    const ref = this.modalService.open(AddScheduleSmsModelComponent);
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = true;
        console.log(res);
