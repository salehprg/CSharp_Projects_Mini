...
export class AddUserComponent implements OnInit {
    ...
    this.userService.getUserById(this.userId).subscribe(
      res => {
        if (res && res.Status && res.Status.length === 1 && res.Status[0].status === 200) {
          ...
          if (this.user.addressInfo) {
            this.mapService.setAddress(this.user.addressInfo.longitude,
              this.user.addressInfo.latitude);
            this.address = this.user.addressInfo.region + ' - ' + this.user.addressInfo.city + ' - ' + this.user.addressInfo.address;
          }
          this.initForm();
        } else {
  ...
  AccountInfoExit() {
    this.user.accountInfo = {
      id: this.isEdit ? this.user.accountInfo.id : undefined,
      ...
      this.userService.editUser(this.user).subscribe(
        (res: ResponseModel) => {
          if (res.Status.length === 1 && res.Status[0].status === 200) {
            ...
            this.router.navigate(['/userManagement/listUser']);
          } else {
        ...
        err => {
          this.toaster.error('لطفا مجددا تلاش کنید.', 'خطا');
        }
      );
      return;
    }
    this.user.roles = map(this.roleIds, (itm) => {
      return itm.id;
    });
