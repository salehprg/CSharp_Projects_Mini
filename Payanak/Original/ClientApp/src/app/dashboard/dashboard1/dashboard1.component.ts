...
export class Dashboard1Component implements OnInit {
  ...
  ngOnInit(): void {
    this.taskService.getDashboardInfo().subscribe(
      res => {
        if (res.Status && res.Status.length === 1 && res.Status[0].status === 200) {
          this.data = res.Result;
          this.lineArea.data = {
            ...this.lineArea.data,
            series: [res.Result.Contacts10Day.reverse()]
          };
          this.lineArea2.data = {
            ...this.lineArea.data,
            series: [res.Result.CountCalc10Day.reverse(), res.Result.Count10Day.reverse()]
          };
        } else {
