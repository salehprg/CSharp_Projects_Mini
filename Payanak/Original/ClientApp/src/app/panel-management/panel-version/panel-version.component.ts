...
@Component({
  selector: 'app-panel-version',
  templateUrl: './panel-version.component.html',
  styleUrls: ['./panel-version.component.scss']
})
export class PanelVersionComponent implements OnInit, OnDestroy {

  queryParam: QueryParamModel;
  versions: PanelVersionModel[] = [];
  length = 0;
  pageSize = 10;
  ...
  loadData() {
    ...
    this.loading = true;
    this.panelService.getPanelVersions(this.queryParam).pipe(
      take(1))
      .subscribe(
        res => {
  ...
  delete(version: PanelVersionModel) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.loading = true;
        this.panelService.deletePanelVersion(version.id).subscribe(
          res => {
            this.loading = false;
  ...
  lunchAddPanelVersion() {
    const ref = this.modalService.open(AddPanelVersionModalComponent, {size: 'lg'});
    ref.result.then(res => {
      if ((typeof (res)) !== 'string') {
        this.loading = true;
        this.panelService.addPanelVersion(res.versionModel, res.files[0]).subscribe(
          res => {
            this.loading = false;
            this.successAndLoad(res);
          }
